				...
				openClientAccount(function(binData) {
					...
					oTools.SaveToLocal({
						...
						if (getUrlParameter("type") === '1') {
							...
							'&&mobileNo=' + oData.MOBILENO + '&&branchNo=' + branchNo + '&&clientId=' + oData.CLIENTID + '&&clientName=' + oData.CLIENTNAME);
							return;
						} else {
							//window.location.href = 'hand.htm';
...
function openClientAccount(succ) {
	...
	var params = cRequestSign + encodeURI("?action=31001&path=/WA0017&op_station=($tztudid)&branch_no=" + branchNo +
...
function checkIdno(succ) {
	...
	var params = cRequestSign + encodeURI("?action=31001&path=/WA0013&op_station=($tztudid)&branch_no=" + branchNo +
